...
export class EnhancedAuthController {
  ...
  async registerEnhanced(@Body(ValidationPipe) registerDto: EnhancedRegisterDto) {
  ...
  async updateVehicleProfile(
    ...
    return this.enhancedAuthService.updateVehicleProfile(req.user.id, vehicleData);
  }
}
