...
export class MarketplaceChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  @WebSocketServer()
  server: Server;
  ...
  async handleMessage(
    ...
    try {
      ...
      const recipientId = chat.buyerId === data.userId ? chat.sellerId : chat.buyerId;
