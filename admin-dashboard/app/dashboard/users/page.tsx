...
export default function UsersPage() {
  ...
  const handleViewPaymentAccounts = async (user: User) => {
    if (user.role !== 'owner') {
      toast.info('Only owners have payment accounts');
      return;
    }
  ...
  const getVerificationBadge = (status: string) => {
    switch (status) {
      ...
      case 'rejected':
        return (
          <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300">
      ...
      default:
        return (
          <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300">
                      ...
                      <td className="px-6 py-4 text-right space-x-2">
                        {actionLoading === user.id ? (
                          ...
                          <div className="flex justify-end gap-2">
                            {/* View Bank Details Button (for owners only) */}
                            {user.role === 'owner' && (
                              <button
                                onClick={() => handleViewPaymentAccounts(user)}
                                className="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-200 dark:border-indigo-700 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-all"
              ...
              {loadingAccounts ? (
                ...
                <div className="space-y-4">
                  {paymentAccounts.map((account) => (
                    <Card key={account.id} className="border-slate-200 dark:border-slate-700">
                      <CardContent className="p-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-900 dark:text-slate-100 mt-1 capitalize">
                              {account.accountType}
                            </p>
                          </div>
                          ...
                          {account.verificationNotes && (
                            <div className="col-span-2">
                              ...
                              <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 bg-slate-50 dark:bg-slate-900/50 p-3 rounded-lg">
                                {account.verificationNotes}
                              </p>
                            </div>
                          )}
