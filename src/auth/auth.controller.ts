...
export class AuthController {
  ...
  async login(@Body() loginDto: LoginDto) {
    try {
      console.log(
        "[AUTH CONTROLLER] Login request received for:",
        loginDto.phoneNumber,
      );
      const result = await this.authService.login(loginDto);
      console.log(
        "[AUTH CONTROLLER] Login successful for:",
        loginDto.phoneNumber,
      );
      return result;
    } catch (error) {
  ...
  async getCurrentUser(@Request() req) {
    ...
    console.log(
      "🔍 /auth/me: Checking chargers for user:",
      user.phoneNumber,
      "ID:",
      user.id,
    );
    ...
    console.log(
      "   - Calculated isOwner:",
      user.role === "owner" || user.role === "admin" || hasChargers,
    );
    console.log(
      "   - Calculated isMechanic:",
      hasMechanicProfile || user.role === "mechanic" || user.role === "admin",
    );
