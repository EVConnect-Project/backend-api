...
import {
  ...
  ChargerStopDto,
  SafetyWarningDto,
} from "../dto/route-alternative.dto";
...
export class SmartTripPlannerService {
  ...
  async resolveVehicleIdForUser(
    ...
    if (!fallbackVehicle) {
      throw new BadRequestException(
        "No vehicle found for this user. Add a vehicle profile first.",
      );
    }
  ...
  async generateSmartRoutes(
    ...
    const drivingMode =
      dto.drivingMode || vehicle.drivingMode || DrivingMode.NORMAL;
    ...
    const preferredNetworks = this.normalizeNetworkKeywords(
      dto.preferredNetworks,
    );
    const excludedNetworks = this.normalizeNetworkKeywords(
      dto.excludedNetworks,
    );
  ...
  private async getGoogleDirections(
    ...
    const fallback = () =>
      this.getFallbackRoute(startLat, startLng, destLat, destLng, waypoints);

    if (!this.GOOGLE_MAPS_API_KEY) {
      this.logger.warn(
        "GOOGLE_MAPS_API_KEY not configured. Using OSRM fallback geometry.",
      );
      return fallback();
    }
    ...
    try {
      ...
      this.logger.warn(
        `Google Directions API returned: ${response.data.status}`,
      );
      return fallback();
    } catch (error) {
  ...
  private async processRoute(
    ...
    const adjustedConsumption =
      (vehicle.averageConsumption || 180) *
      modeMultiplier *
      energyAdjustment.multiplier;
    ...
    this.logger.log(
      `Route ${routeNumber}: ${totalDistanceKm.toFixed(1)}km, Range: ${usableRange.toFixed(1)}km, Mode: ${drivingMode}`,
    );
    ...
    if (needsCharging) {
      ...
      finalBatteryPercent =
        currentBatteryPercent - (energyUsedKwh / vehicle.batteryCapacity) * 100;
    ...
    if (totalTrafficDurationS > totalDurationS * 1.2) {
      const delayMinutes = Math.round(
        (totalTrafficDurationS - totalDurationS) / 60,
      );
      safetyWarnings.push({
        type: "traffic",
    ...
    const totalChargingCostLkr = chargingStops.reduce(
      (sum, s) => sum + s.estimatedCostLkr,
      0,
    );
    ...
    return {
      ...
      energyAdjustmentPercent: Math.max(
        0,
        (energyAdjustment.multiplier - 1) * 100,
      ),
  ...
  private calculateEtaConfidencePercent(
    ...
    const hasAvailabilityRisk = warnings.some(
      (w) => w.type === "charger_availability_risk",
    );
    ...
    if (stops.length > 0) {
      const avgReliability =
        stops.reduce((sum, stop) => sum + (stop.reliabilityScore || 0.0), 0) /
        stops.length;
  ...
  private calculateSocConfidencePercent(
    ...
    const energyAdjustmentPercent = Math.max(
      0,
      (energyAdjustment.multiplier - 1) * 100,
    );
  ...
  private calculateRouteScore(
    ...
    switch (routeObjective) {
      ...
      case RouteObjective.CHEAPEST:
        return (
          costInHundreds * 0.45 +
          totalChargingTimeMinutes * 0.2 +
          stopPenalty * 0.2 +
          drivingDurationMinutes * 0.1 +
          totalDistanceKm * 0.05 +
          uncertaintyPenalty
        );
  ...
  private calculateUncertaintyPenalty(
    ...
    const adjustmentPercent = Math.max(
      0,
      (energyAdjustment.multiplier - 1) * 100,
    );
  ...
  private async findOptimalChargingStops(
    ...
    const pathPoints = this.decodePolyline(
      googleRoute.overview_polyline?.points || "",
    );
    const segments = this.createRouteSegments(
      pathPoints,
      this.ROUTE_SEGMENT_LENGTH,
    );

    let currentBatteryKwh =
      (vehicle.batteryCapacity * initialBatteryPercent) / 100;
    ...
    const routeChargers = await this.loadChargersAlongRoute(
      pathPoints,
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      const currentBatteryPercent =
        (currentBatteryKwh / vehicle.batteryCapacity) * 100;
      ...
      if (currentBatteryPercent < minChargeAtChargingStationPercent) {
        ...
        const availabilityConfidencePercent = Math.round(
          Math.max(
            35,
            Math.min(96, this.getAvailabilityQuality(bestCharger) * 100),
          ),
        );
        ...
        const targetBatteryKwh =
          vehicle.batteryCapacity * (targetBatteryPercent / 100);
        const energyToChargeKwh = targetBatteryKwh - currentBatteryKwh;

        const chargingPowerKw = this.getEffectiveChargingPower(
          bestCharger,
          vehicle,
        );
        ...
        stops.push({
          ...
          googleMapUrl:
            bestCharger.googleMapUrl ||
            `https://www.google.com/maps?q=${bestCharger.lat},${bestCharger.lng}`,
          ...
          chargingPowerKw,
          estimatedCostLkr: Math.round(
            energyToChargeKwh * (Number(bestCharger.pricePerKwh) || 35),
          ),
    ...
    if (preferredFallbackUsed) {
      warnings.push({
        ...
        severity: "low",
        message:
          "Preferred charging networks were not consistently available on this route. Alternatives were used where needed.",
      });
    }
    ...
    if (availabilityFallbackUsed) {
      warnings.push({
        ...
        severity: "medium",
        message:
          "Some selected charging stops may have lower real-time availability. Consider backup stations along the route.",
      });
    }

    const totalChargingTime = stops.reduce(
      (sum, stop) => sum + stop.chargingTimeMinutes,
      0,
    );
    const finalBatteryPercent =
      (currentBatteryKwh / vehicle.batteryCapacity) * 100;
  ...
  private async loadChargersAlongRoute(
    ...
    const lngBuffer =
      bufferKm / (111.0 * Math.cos((((minLat + maxLat) / 2) * Math.PI) / 180));
    ...
    try {
      ...
      this.logger.log(
        `Loaded ${chargers.length} ${this.CHARGER_SOURCE} within route bounding box`,
      );
      return chargers;
    } catch (error) {
  ...
  private filterNearbyCompatibleChargers(
    ...
    return allChargers.filter((charger) => {
      if (this.matchesAnyNetworkKeyword(charger, excludedNetworks))
        return false;
      ...
      return this.isConnectorCompatible(
  ...
  private async buildEnergyAdjustmentContext(
    ...
    const elevationPenalty = this.calculateElevationPenalty(
      dto.elevationDeltaM,
    );
    const hvacPenalty = Math.min((dto.hvacLoadKw ?? 0.0) * 0.02, 0.08);
    const multiplier = Math.max(
      0.85,
      1 + weatherPenalty + elevationPenalty + hvacPenalty,
    );
  ...
  private matchesAnyNetworkKeyword(
    charger: Charger,
    keywords: string[],
  ): boolean {
    ...
    const searchText =
      `${charger.name || ""} ${charger.address || ""}`.toLowerCase();
   ...
   */
  private isConnectorCompatible(
    chargerConnector: string | null,
    vehicleConnector: string,
  ): boolean {
    if (!chargerConnector) return false;

    const vehicleConnectors = vehicleConnector
      .toLowerCase()
      .split(",")
      .map((c) => c.trim());
    const chargerConn = chargerConnector.toLowerCase();

    return vehicleConnectors.some(
      (vc) => vc.includes(chargerConn) || chargerConn.includes(vc),
    );
  }
  ...
  private rankChargersForStop(
    ...
    return [...chargers].sort((a, b) => {
      ...
      const corridorDistA = this.distanceToRouteKm(
        Number(a.lat),
        Number(a.lng),
        routePoints,
      );
      const corridorDistB = this.distanceToRouteKm(
        Number(b.lat),
        Number(b.lng),
        routePoints,
      );
      ...
      const scoreA =
        (distA / maxDist) * 0.2 +
        (corridorDistA / maxCorridorDist) * 0.2 +
        (1 - powerA / maxPower) * 0.25 +
        (1 - reliabilityA) * 0.15 +
        (1 - availabilityA) * 0.2;
      const scoreB =
        (distB / maxDist) * 0.2 +
        (corridorDistB / maxCorridorDist) * 0.2 +
  ...
  private calculateChargingTime(
    ...
    const curve =
      vehicle.chargingCurve && vehicle.chargingCurve.length > 0
        ? vehicle.chargingCurve.map((p) => ({
            percentage: p.percentage,
            factor: p.powerKw / maxChargingPowerKw,
          }))
  ...
  private interpolateChargingFactor(
    ...
    for (let i = 0; i < curve.length - 1; i++) {
      if (
        percent >= curve[i].percentage &&
        percent <= curve[i + 1].percentage
      ) {
        lower = curve[i];
        upper = curve[i + 1];
        break;
    ...
    const t =
      (percent - lower.percentage) / (upper.percentage - lower.percentage);
