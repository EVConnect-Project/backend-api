...
export class TripPlannerService {
  ...
  async planRoute(planRouteDto: PlanRouteDto, userId: string): Promise<RouteResponseDto> {
    const { start, destination, currentBattery = 80, minBatteryAtStop = 20 } = planRouteDto;
    ...
    const { distance, duration, polyline } = await this.calculateRoute(startCoords, endCoords);
    ...
    const energyNeeded = this.calculateEnergyConsumption(distance, vehicleSpecs);
    ...
    return {
      ...
      recommendedDepartureBattery: this.calculateRecommendedBattery(distance, vehicleSpecs),
    };
  }
  ...
  private calculateEnergyConsumption(distanceKm: number, specs: VehicleSpecs): number {
  ...
  private async calculateChargingStops(
    ...
    while (remainingDistance > maxRangeKm) {
      ...
      const energyConsumed = this.calculateEnergyConsumption(distanceToCharger, vehicleSpecs);
      const batteryConsumed = (energyConsumed / vehicleSpecs.batteryCapacity) * 100;
      ...
      const energyNeeded = ((targetChargeLevel - batteryOnArrival) / 100) * vehicleSpecs.batteryCapacity;
  ...
  private async findChargersAlongRoute(
    ...
    return this.chargerRepository.query(query, [targetLat, targetLng, radiusKm]);
  }
  ...
  private selectBestCharger(chargers: Charger[]): Charger {
    ...
    for (const charger of chargers) {
      ...
      const score = powerScore * 0.4 + priceScore * 0.3 + availabilityScore * 0.3;
  ...
  private calculateRecommendedBattery(distance: number, specs: VehicleSpecs): number {
