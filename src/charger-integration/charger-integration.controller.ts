...
export class ChargerIntegrationController {
  ...
  async generateCredentials(
    ...
    this.logger.log(
      `User ${req.user.userId} requesting OCPP credentials for charger ${chargerId}`,
    );
  ...
  async chargerConnected(
    ...
    await this.integrationService.linkOcppCharger(
      body.chargeBoxIdentity,
      body.ocppChargerId,
    );
    return { success: true };
  }
  ...
  async sessionCompleted(@Body() body: { sessionId: string }) {
    ...
    const revenue = await this.integrationService.distributeSessionRevenue(
      body.sessionId,
    );
