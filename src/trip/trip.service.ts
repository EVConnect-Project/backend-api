...
export class TripService {
  private readonly AI_SERVICE_URL = process.env.AI_SERVICE_URL || 'http://localhost:5000';

  constructor(private readonly httpService: HttpService) {}

  async createTripPlan(createTripPlanDto: CreateTripPlanDto): Promise<TripPlanResponseDto> {
    try {
  ...
  private transformAIResponse(data: any): TripPlanResponseDto {
    return {
      ...
      chargingStops: data.charging_stops?.map((stop: any) => ({
      ...
      routeSegments: data.route_segments?.map((segment: any) => ({
      ...
      safetyAlerts: data.safety_alerts?.map((alert: any) => ({
        type: alert.type,
