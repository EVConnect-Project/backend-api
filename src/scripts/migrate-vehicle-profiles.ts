...
async function migrateVehicleProfiles() {
  const dataSource = new DataSource({
    ...
    url:
      ...
      "postgresql://akilanishan@localhost:5432/evconnect",
  ...
  try {
    ...
    console.log(
      `Found ${usersWithVehicleData.length} users with vehicle data in UserEntity`,
    );
    ...
    for (const user of usersWithVehicleData) {
      ...
      if (existingProfile) {
        console.log(
          `User ${user.name} (${user.id}) already has a vehicle profile, skipping`,
        );
        continue;
      }
      ...
      const vehicleProfile = vehicleProfileRepository.create({
        ...
        connectorType:
          user.connectorTypes && user.connectorTypes.length > 0
            ? user.connectorTypes[0]
            : "Type2",
      ...
      migratedCount++;
      console.log(
        `✅ Created vehicle profile for user ${user.name} (${user.id})`,
      );
    }
